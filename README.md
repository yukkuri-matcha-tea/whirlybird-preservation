...
したがって、現段階は「エミュレーターで起動・START後の継続動作を確認済み」であり、「全項目を実機で完全検証済み」ではありません。
...
スクリプトはAPKの導入、cold launch、START操作、プロセス生存、既知の重大エラーを確認します。センサー、音、見た目、本家比較は人による実機確認が必要です。
...
現状の優先順位は、元コードと元データを維持した動作保存、現行Android互換性、再現可能なビルド、そして実機比較です。
